...
angular.module('ui.layout', [])
  .controller('uiLayoutCtrl', [
    ...
    '$timeout',
    '$window',
    'LayoutContainer', 'LayoutStateService', function uiLayoutCtrl(
      $scope,
      $attrs,
      $element,
      $timeout,
      ...
      //================================================================================
      ctrl.mouseUpHandler = function (event, layoutStateService) {
        if (ctrl.movingSplitbar !== null) {
          ctrl.movingSplitbar = null;
        }
        layoutStateService.persist();
        return event;
      };
        ...
        if (ctrl.containers.length > 0 && $element.children().length > 0) {
          ...
          for (i = 0; i < ctrl.containers.length; i++) {
            ...
            if (!LayoutContainer.isSplitbar(ctrl.containers[i])) {
              ...
              var containerState = LayoutStateService.getSize(c.containerId);
              if (containerState !== undefined && containerState !== null) {
                opts.sizes[i] = containerState.containerSize + 'px';
              } else {
  ...
  .directive('uiSplitbar', [
    'LayoutContainer',
    'LayoutStateService', function (
      LayoutContainer,
      LayoutStateService) {
    // Get all the page.
    ...
    return {
      ...
      link: function (scope, element, attrs, ctrl) {
        ...
        //icon classes
        var iconLeft = 'glyphicon-chevron-left';
        var iconRight = 'glyphicon-chevron-right';
        ...
        htmlElement.on('mouseup touchend', function (event) {
          scope.$apply(angular.bind(ctrl, ctrl.mouseUpHandler, event, LayoutStateService));
        ...
        //Add splitbar to layout container list
          scope.splitbar.containerId = LayoutStateService.getNextId();
        ctrl.addContainer(scope.splitbar);
          LayoutStateService.push(scope.splitbar);
      ...
      compile: function () {
        return {
          pre: function (scope, element, attrs, ctrl) {
            ...
            scope.container.containerId = LayoutStateService.getNextId();
            ctrl.addContainer(scope.container);
            LayoutStateService.push(scope.container);
  ...
  .value('uiLayoutStateKey', 'uiLayoutState')
  .service('LayoutStateService', [
    '$state',
    '$timeout',
    'storage',
    'uiLayoutStateKey', function (
      $state,
      $timeout,
      storage,
      uiLayoutStateKey) {
      ...
      self.getCurrentStateKey = function () {
        return uiLayoutStateKey + '.' + $state.current.name;
      }
      ...
      self.getSize = function (containerId) {
        return _.find(JSON.parse(storage.get(self.getCurrentStateKey())), function (container) {
      ...
      self.getPersistedInfo = function () {
        return containers;
      }

      self.serialize = function () {
        return JSON.stringify(_.map(containers, function (container) {
          return { containerSize: container.size, containerId: container.containerId };
        }));
      }
      ...
      self.persist = function () {
        ...
        timer = $timeout(function () {
          storage.set(self.getCurrentStateKey(), self.serialize());
          timer = null;
        }, 50);
      }
    }])
